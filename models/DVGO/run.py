...
def config_parser():
    ...
    parser.add_argument("--s3im_weight", default=0.0, type=float)
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            src_rgb = utils.to8b(gt_imgs[i])
            rgb8 = utils.to8b(rgbs[i])
            src_filename = os.path.join(savedir, 'src_{:03d}.png'.format(i))
            filename = os.path.join(savedir, 'tar_{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)
            imageio.imwrite(src_filename, src_rgb)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    s3im_func = S3IM(patch_height=cfg_train.s3im_patch_height, patch_width=cfg_train.s3im_patch_width, kernel_size=cfg_train.s3im_kernel, stride=cfg_train.s3im_stride, repeat_time=cfg_train.s3im_repeat_time).to(device)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        loss += mse_loss
        if cfg_train.s3im_weight > 0:
            s3im_loss = s3im_func(render_result['rgb_marched'], target) * cfg_train.s3im_weight
            loss += s3im_loss
